...
SELECT language FROM countrylanguage WHERE countrycode = (SELECT code FROM country WHERE name = 'Holy See (Vatican City State)');



-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. Find out which nearby country speaks nothing but that language.

SELECT name FROM country WHERE code IN (SELECT countrycode FROM countrylanguage WHERE language = 'Italian' AND region = 'Southern Europe'AND percentage = '100') AND name != 'Holy See (Vatican City State)';
...
SELECT name FROM city WHERE countrycode IN (SELECT code From country WHERE region = 'South America') AND name LIKE 'Serr%';



-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towards the capital! Look up the country's capital, and get there pronto! Send us the name of where you're headed and we'll follow right behind you!

SELECT name FROM city WHERE id = (SELECT capital FROM country WHERE code = (SELECT countrycode FROM city WHERE name = 'Serra'));
